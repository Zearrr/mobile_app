...
const Claims = () => {
  ...
  const perPage = 6;
  const totalPages = Math.max(1, Math.ceil(claimableJobs.length / perPage));
  const start = (page - 1) * perPage;
  const pageItems = claimableJobs.slice(start, start + perPage);

  // Reset page when filters or view change
  useEffect(() => {
    setPage(1);
  }, [searchPhone, searchName, searchCode, viewMode]);

  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      {/* Page Header - single blue with action buttons */}
      <div className="rounded-2xl bg-blue-600 text-white shadow-xl p-5 md:p-6 flex items-center justify-between mb-8">
        ...
        <div className="hidden sm:flex items-center gap-3">
          <Link to="/warranty/new">
            <Button className="rounded-xl bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 shadow-lg">
              เพิ่มเครมใหม่
            </Button>
          </Link>
          ...
          {claimableJobs.length > 0 && (
            <div className="flex items-center justify-between p-4 border-t">
              <div className="text-sm thai-text text-muted-foreground">
                หน้า {page} / {totalPages} • แสดง {pageItems.length} จาก {claimableJobs.length} รายการ
              </div>
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
