...
export default function NewJob() {
  ...
  const onSubmit = async (data: NewJobForm) => {
    ...
    try {
      ...
      // ไปหน้าพิมพ์ใบแจ้งซ่อมสองฝั่ง A4 แนวนอนทันที
                          ...
                          <div className="flex items-center gap-2">
                            <Popover open={openMultiParts} onOpenChange={setOpenMultiParts}>
                              <PopoverTrigger asChild>
                                <Button type="button" variant="outline" role="combobox" aria-expanded={openMultiParts} className="w-full h-10 text-sm rounded-lg justify-between thai-text">
                                  เลือกอะไหล่...
                                  <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                </Button>
                              </PopoverTrigger>
                              <PopoverContent side="bottom" align="start" sideOffset={4} className="p-0 w-[var(--radix-popover-trigger-width)] z-50 max-h-60 overflow-auto bg-white border shadow-lg">
                            ...
                            <Button
                               type="button"
                               className="h-10 text-sm rounded-lg px-4 thai-text"
...
// Bottom action bar (fixed) added below the component render - using form="new-job-form" to submit
export function NewJobBottomBar() {
  return (
    <div className="print:hidden fixed bottom-0 left-0 right-0 bg-white/95 border-t py-3 flex items-center justify-center gap-3 z-50">
      <Button
        type="button"
        variant="outline"
      ...
      <Button type="submit" form="new-job-form" className="btn-primary">
        <Save className="w-4 h-4 mr-2" /> บันทึกการแจ้งซ่อม
      </Button>
    </div>
  );
}
